...
namespace SpawnMobXY
{
    ...
    public class Plugin : TerrariaPlugin
    {
        ...
        public override string Description => "Spawn mobs with special properties and personalized drops";
        ...
        // Dictionary for storing custom NPC drops
        private static readonly Dictionary<int, (int itemId, int amount, int chance)> customDrops =
            new Dictionary<int, (int itemId, int amount, int chance)>();
        ...
        private static void SpawnMobWithParameters(int type, int x, int y, IDictionary<string, string> parameters)
        {
            ...
            if (parameters.ContainsKey("drop"))
            {
                var parts = parameters["drop"].Split(':');
                int itemId = int.Parse(parts[0]);
                int amount = parts.Length > 1 ? int.Parse(parts[1]) : 1;
                int chance = parts.Length > 2 ? int.Parse(parts[2]) : 100; // default 100%

                customDrops[npcIndex] = (itemId, amount, chance);
            }
        ...
        private void OnNpcKilled(NpcKilledEventArgs args)
        {
            if (customDrops.TryGetValue(args.npc.whoAmI, out var drop))
            {
                ...
                var rand = new Random();
                int roll = rand.Next(1, 101); // between 1 and 100

                // If the roll is within the defined probability, the drop is generated.
                if (roll <= drop.chance)
                {
                    Item.NewItem(
    args.npc.GetItemSource_Loot(),
    args.npc.position,
    args.npc.width,
    args.npc.height,
    drop.itemId,
    drop.amount
                ...
                // Clean the dictionary to avoid accumulating data
